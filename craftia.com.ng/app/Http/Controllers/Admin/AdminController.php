...
class AdminController extends Controller
{
    ...
    public function dashboard() {
        ...
        // If the user is a vendor, get counts specific to that vendor
        if ($adminType === 'vendor') {
            // Assuming you have a relationship set up for products, orders, etc. for the vendor
            $productsCount = Product::where('vendor_id', $loggedInAdmin->vendor_id)->count();
            $ordersCount = Order::where('user_id', $loggedInAdmin->vendor_id)->count();    
             $ordersCount = DB::table('orders_products')
                            ->where('vendor_id', $loggedInAdmin->id)
                            ->count('order_id');
            $couponsCount = Coupon::where('vendor_id', $loggedInAdmin->vendor_id)->count();
            // dd($loggedInAdmin->vendor_id,$couponsCount);
                       
                            
            // Adjust other counts as necessary
        }
        ...
        if ($slug == 'personal') {
            ...
            if ($request->isMethod('post')) { // if the <form> is submitted
                ...
                $rules = [
                    ...
                    'nin_number'            =>'required|min:11|max:11',
