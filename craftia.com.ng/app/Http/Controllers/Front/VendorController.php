        ...
        if ($request->isMethod('post')) { // if the register form is submitted
            ...
            $rules = [
                // <input> "name" attribute => its rule
                            ...
                            'mobile'        => 'required|min:11|numeric|unique:admins|unique:vendors', // 'unique:admins' and 'unique:vendors' means check the `admins` table and `vendors` table for the `mobile` uniqueness: https://laravel.com/docs/9.x/validation#rule-unique    // 'min:10|numeric' is the mobile number validation
                            'accept'        => 'required'
            ];
            ...
            date_default_timezone_set('Africa/Lagos'); // https://www.php.net/manual/en/timezones.php and https://www.php.net/manual/en/timezones.africa.php
